...
def pin_memory(data_queue, pinned_data_queue, sema):
    while True:
        ...
        data["xs"] = [[x.pin_memory() for x in z] for z in data["xs"]]
        data["ys"] = [[y.pin_memory() for y in z] for z in data["ys"]]
...
def train(training_dbs, validation_db, start_iter=0):
    ...
    sample_data_1 = importlib.import_module(data_file).sample_data

    # allocating resources for parallel reading
    training_tasks = init_parallel_jobs(training_dbs, training_queue, sample_data, True)
    if val_iter:
        validation_tasks = init_parallel_jobs([validation_db], validation_queue, sample_data_1, False)
