...
from .resnet_features import rfdnet, resnet50_features, resnet152_features, resnet18_features, resnet101_features, resnext101_32x8d, wide_resnet101_2
...
class SubNet(nn.Module):
    ...
    def __init__(self, mode, classes=2, depth=4,
        ...
        #! 修改通道数128
        # self.subnet_base = nn.ModuleList([conv3x3(128, 128, padding=1)
        #                                   for _ in range(depth)])

        # if mode == 'corners':
        #     self.subnet_output = conv3x3(128, 2, padding=1)

        # if mode == 'centers':
        #     self.subnet_output = conv3x3(128, 2 , padding=1)
        # elif mode == 'classes':
        #     # add an extra dim for confidence
        #     self.subnet_output = conv3x3(128, self.classes, padding=1)
            
        #! 修改通道数192   
        # self.subnet_base = nn.ModuleList([conv3x3(192, 192, padding=1)
        #                                   for _ in range(depth)])

        # if mode == 'corners':
        #     self.subnet_output = conv3x3(192, 2, padding=1)

        # if mode == 'centers':
        #     self.subnet_output = conv3x3(192, 2 , padding=1)
        # elif mode == 'classes':
        #     # add an extra dim for confidence
        #     self.subnet_output = conv3x3(192, self.classes, padding=1)
            
        #! 修改通道数96   
        self.subnet_base = nn.ModuleList([conv3x3(96, 96, padding=1)
                                          for _ in range(depth)])

        if mode == 'corners':
            self.subnet_output = conv3x3(96, 2, padding=1)

        if mode == 'centers':
            self.subnet_output = conv3x3(96, 2 , padding=1)
        elif mode == 'classes':
            # add an extra dim for confidence
            self.subnet_output = conv3x3(96, self.classes, padding=1)
        
        #! 通道数64
        # self.subnet_base = nn.ModuleList([conv3x3(64, 64, padding=1)
        #                                   for _ in range(depth)])

        # if mode == 'corners':
        #     self.subnet_output = conv3x3(64, 2, padding=1)

        # if mode == 'centers':
        #     self.subnet_output = conv3x3(64, 2 , padding=1)
        # elif mode == 'classes':
        #     # add an extra dim for confidence
        #     self.subnet_output = conv3x3(64, self.classes, padding=1)
...
class MatrixNetCorners(nn.Module):
    def __init__(self, classes, resnet, layers):
        ...
        elif self.resnet == "rfdnet":
            _resnet = rfdnet()
        try: 
            self.feature_pyramid = MatrixNet(_resnet, layers)
        except : 
            print("ERROR: ivalid resnet") ## 每次这里报错？金字塔没有实例化出来->layers不对->config里面的layer_range
            sys.exit()
...
class MatrixNetCornerLoss(nn.Module):
    ...
    # def __init__(self, corner_regr_weight=1, center_regr_weight=0, focal_loss=_neg_loss):
    ...
    def forward(self, outs, targets):
        ...
        # print('self.center_regr_weight = ', self.center_regr_weight)
        if numr > 0:
...
def _decode(
    ...
    #top_scores, top_inds = torch.topk(detections[:, :, 4], num_dets)
    top_scores, top_inds = torch.topk(detections[:, :, 4], 5)
