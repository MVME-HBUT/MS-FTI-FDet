...
def kp_decode(nnet, images, K, matching_threshold=0.5, kernel=3, layers_range = None, output_kernel_size = None,output_sizes=None, input_size = None, base_layer_range = None):
    ...
    return detections #dist_threshold
...
def test_MatrixNetCorners(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    ...
    if db.split != "trainval":
        db_inds = db.db_inds[:850] if debug else db.db_inds #定义debug检测多少图片 200->2022
    else:
        db_inds = db.db_inds[:100] if debug else db.db_inds[:100] #100->2022
    ...
    result_txt = os.path.join(result_dir, "results.txt")  #定义txt文件路径
    result_txt1 = os.path.join(result_dir, "results1.txt")
    top_bboxes = {}
    
    ## zy add
    # images  = np.zeros((1, 3, 20, 20), dtype=np.float32)
    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        if debug:
            ...
            #print(top_bboxes[image_id][1][keep_inds])
            #np.savetxt(result_txt, top_bboxes[image_id][1][keep_inds])

            #写入结果的txt文件
            # arr_top_bboxes = top_bboxes[image_id][1][keep_inds]
            # f1 = open(result_txt1,'a')
            # f1.write(str(arr_top_bboxes))
            # #f1.write(top_bboxes[image_id][1][keep_inds])
            # f1.write('\n')
            # f1.close()
    ...
    #detections = db.my_convert_to_coco(top_bboxes)

    #print(detections[0]["bbox"][0])
    # f1 = open(result_txt,'a')
    # for i in detections:
    #     my_box0 = i["bbox"][0]
    #     my_box1 = i["bbox"][1]
    #     my_box2 = i["bbox"][2]
    ...
    #     my_score = i["score"]
    #     #print(my_score)
    #     f1.write(str(my_box0) + ' ' +str(my_box1) + ' '+str(my_box2) + ' '+str(my_box3) + ' ' +str(my_score))
    #     f1.write('\n')
    # f1.close()


    #detections  = db.convert_to_coco(top_bboxes[image_id][1])
...
def test_MatrixNetAnchors(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    ...
    result_txt = os.path.join(result_dir, "results.txt")
    top_bboxes = {}
    #for ind in tqdm(range(0, num_images),miniters = int(1e6), ncols=80, desc="locating kps",mininterval = 0.001):
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        db_ind = db_inds[ind]
