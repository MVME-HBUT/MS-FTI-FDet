...
class MSCOCO(DETECTION):
    def __init__(self, db_config, split):
        ...
        self._image_dir  = os.path.join(self._coco_dir, "img_test") #img or img_test  #! 注意在测试和训练过程中修改数据集,否则测试结果不准确！

        # if self._dataset == "testdev2017":
        #     self._image_dir  = os.path.join(self._coco_dir, "img_test") ##img or img_test
        #     print("****************************************************")
        #     print(self._image_dir)
        #     print("****************************************************")
        # else:
        #     self._image_dir  = os.path.join(self._coco_dir, "img")
        #     print("****************************************************")
        #     print(self._image_dir)
        #     print("****************************************************")
        ...
        self._cache_file = os.path.join(cache_dir, "coco_{}.pkl".format(self._dataset))#coco_{}
        self._load_data()
    ...
    def my_convert_to_coco(self, all_bboxes): #有缺陷，不能正确识别
        detections = []
        ...
        for image_id in all_bboxes:
            ...
            for cls_ind in all_bboxes[image_id]:
                category_id = self._classes[cls_ind]
                bbox = all_bboxes[image_id][cls_ind][0]
                bbox[2] -= bbox[0]
                bbox[3] -= bbox[1]
                score = bbox[4]
                bbox  = list(map(self._to_float, bbox[0:4]))
